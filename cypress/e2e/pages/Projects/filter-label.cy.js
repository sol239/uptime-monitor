...
describe('Laravel + Inertia Login', () => {
    it('Logs in with valid credentials', () => {
        ...
        // FILTER BY TAGS: "cyp"
        cy.get('input[placeholder="Filter by label"]').type('Shadow');

        // Check that Project 1 and 2 are visible, Project 3 is not
        cy.contains('Shadow Libraries').should('be.visible');
        cy.contains('My Test Project 2').should('not.exist');
        cy.contains('My Test Project 3').should('not.exist');

        cy.wait(2000);

        cy.get('input[placeholder="Filter by label"]').clear();
