    ...
    <div class="min-h-screen flex items-center justify-center">
      <main class="flex-grow flex items-center justify-center">
        <div class="bg-white dark:bg-[#121212] rounded-xl shadow-lg max-w-4xl w-full p-10"
          ...
          <div class="mb-8">
            <h2 class="text-lg font-bold mb-2">Logs Data (Auto-refreshing every {{ fetchInterval }}s)</h2>
            <div class="flex gap-2 items-center mb-2">
              <button @click="fetchSingleLog"
                class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">
                Refresh Now
              </button>
              <span v-if="logsLoading" class="text-gray-500 ml-2">Loading...</span>
              <span class="text-gray-500 ml-2">{{ logsData.length }} logs loaded</span>
            </div>
            <div v-if="logsData.length > 0"
              class="bg-gray-100 dark:bg-zinc-900 p-4 rounded border mt-2 max-h-48 overflow-y-auto">
              <pre class="text-xs">{{ JSON.stringify(logsData.slice(0, 3), null, 2) }}...{{ logsData.length > 3 ? ` (showing 3 of
      ${logsData.length})` : '' }}</pre>
            </div>
          </div>
        </div>
